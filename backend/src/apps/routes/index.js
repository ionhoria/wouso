...
const { catchAll } = require('../../api/middlewares')
const db = require('../../db')
...
const mountLocalApp = async (name, reqPath) => {
  const mountPath = path.join('/', name)

  logger.debug(`Mounting local app '${name}':'${reqPath}'.`)

  let app = require(reqPath)
...
router.get(
  ...
  catchAll(async (req, res, next) => {
    const apps = Object.keys(appConfig).filter((app) => appConfig[app].active)

    res.json(apps.map((name) => ({ name })))
  })
)
